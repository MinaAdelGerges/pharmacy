...
function App() {
    ...
    try {
        const response = await axios.post('https://localhost:7168/api/Auth/login', {
        ...
        // 3. THE PERMANENT FIX: Save everything to Browser Memory
        ...
        alert(`Welcome back, ${username}! logged in as ${role}`);
    } catch (error) {
    ...
    //discount price engine
    const [dismissedSuggestions, setDismissedSuggestions] = useState([]);

    //tracking the checkout process and delivery details.
    ...
    const addMedicine = async () => {
        if (!formData.name || !formData.activeIngredient || !formData.category || !formData.expiryDate) {
            alert("⚠️ Please fill in all text fields.");
            return;
        }
        try {
    ...
    /*const toggleActiveStatus = async (medicine) => {
        ...
        const actionText = medicine.IsActive ? "De-activate" : "Re-activate";

        if (window.confirm(`Are you sure you want to ${actionText} ${medicine.name}?`)) {
            try {
                // Change 1: Ensure we use the exact ID from the object
                // Change 2: Match the C# property names exactly (Capital I in Id)
                await axios.put(`${API_URL}/${medicine.id}`, {
                    ...medicine,
                    Id: medicine.id,   // Ensure Capital I for C#
                    IsActive: newStatus
                });

                fetchMedicines();
                alert(`✅ ${medicine.name} has been ${newStatus ? 'Enabled' : 'Disabled'}.`);
            } catch (error) {
                console.error("Update Error:", error);
                // This alert shows if the C# side crashes (Status 500)
                alert("❌ Server Error: Check the black C# console for the exact SQL error.");
            }
        }
    };*/
    ...
    //handleRecordPurchase function that actually sends the shipment to the database:
    const handleRecordPurchase = async () => {
        if (!order.medicineId || !order.supplierId || !order.quantity) {
            alert("⚠️ Please select a medicine, a supplier, and a quantity.");
            return;
        }
    ...
    const findAlternatives = async () => {
        ...
        try {
            ...
            if (activeAlts.length === 0) {
                alert("Medicine found, but no active alternatives in stock.");
                setAlternatives([]);
            } else {
    ...
    const checkSafety = async () => {
        try {
            ...
            const response = await axios.get(`https://localhost:7168/api/Medicines/CheckInteractionByMedicine?med1=${med1}&med2=${med2}`);
    ...
    // Fetch Patients from SQL
    const fetchPatients = async () => {
        try {
            const response = await axios.get(PATIENT_API);
            setPatients(response.data);
        } catch (error) { console.error("Patient API Error:", error); }
    };

    // Add New Patient to SQL
    const addPatient = async () => {
        if (!patientPhone) return alert("Phone Number is required!"); // Phone is the only mandatory field
    ...
    // Get Purchase History for a specific Patient from SQL
    ...
    // Opens the form with existing data
    ...
    const updatePatient = async () => {
        try {
            ...
            alert("❌ Update failed. Check if phone number is unique.");
        }
    };
    ...
    // 1. Fetch: Gets all columns from SQL
    const fetchSuppliers = async () => {
        try {
            const response = await axios.get('https://localhost:7168/api/Suppliers');
    ...
    // 2. Add: Sends the full profile to the C# API
    ...
    // 1. Delete: Removes the supplier and updates the local state
    const deleteSupplier = async (id) => {
        // Professional Confirmation Message
        if (window.confirm("⚠️ Are you sure? This will delete the supplier and all their linked purchase history!")) {
            try {
    ...
    // 2. Fetch History: Gets the full shipment log
    ...
    //Function To calculate Trending Products from Customer sales.
    ...
    //handleSupplierEditClick function
    ...
    /*const recordPurchase = async (medicine) => {
        ...
        try {
            ...
            // 2. Set the Invoice data to show the receipt popup
            setCurrentInvoice({
                medicineName: medicine.name,
                ...
                patient: phone || "Walk-in Customer"
            });

            // 3. Refresh the inventory and patient lists
            fetchMedicines();
            fetchPatients();

            alert("✅ Sale Complete! Stock updated and Invoice generated.");
        } catch (error) {
            alert("❌ Error: Patient phone not found. Please register them first.");
        }
    };*/
    ...
    const addToCart = async (newMedicine) => {
        ...
        if (isActive === false) {
            alert(`🚫 ${medName} is currently disabled.`);
            return;
        }

        
        // 2. AUTOMATED CLINICAL GUARD
    ...
    const handleLogout = () => {
        ...
        localStorage.removeItem('savedUserRole'); // Clear this
        localStorage.removeItem('savedUsername'); // Clear this
        setLoginCredentials({ username: '', passwordHash: '', role: 'Staff' }); // Reset form
    };

    // --- SALES VELOCITY LOGIC ---
    // 1. Create a map of how many times each medicine has been sold
    ...
    // 2. Define the "Trending" threshold 
    const TRENDING_THRESHOLD = 5;
    console.log("Calculated Sales Map:", salesCountMap);

    // This function calculates the risk level based on the expiry date
